...
function Update () {
		...
		for (var i : int = 0; i < count; i++) {
			...
			if (lastFingerId == touch.fingerId) {
				...
				if (touchPad) {
					...
					if (!onlyY) position.x = Mathf.Clamp ((touch.position.x - fingerDownPos.x) / (touchZone.width / 2), -1, 1);
					position.y = Mathf.Clamp ((touch.position.y - fingerDownPos.y) / (touchZone.height / 2), -1, 1);
				}
				else {
					// Change the location of the joystick graphic to match where the touch is
					if (!onlyY) position.x = (touch.position.x - guiCenter.x) / guiTouchOffset.x;
	...
	if (!touchPad) {
		...
		if (!onlyY) gui.pixelInset.x = (position.x - 1) * guiTouchOffset.x + guiCenter.x;
		gui.pixelInset.y = (position.y - 1) * guiTouchOffset.y + guiCenter.y;
	}

	if (tapCount == 0) ResetJoystick();
}
